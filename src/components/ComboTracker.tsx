...
export const ComboTracker: React.FC<ComboTrackerProps> = memo(({ combo, lastBonusPoints, compact = false }) => {
  const { multiplier, colorClass, borderClass } = getComboMultiplier(combo);
    ...
    if (combo === 5 || (combo > 5 && combo % 5 === 0)) {
      if (combo >= 20) {
        bannerConfig = {
          ...
          glowShadow: 'shadow-[0_0_30px_rgba(245,158,11,0.9)] ring-2 ring-amber-300',
        };
      } else if (combo >= 10) {
        bannerConfig = {
          comboCount: combo,
          ...
          glowShadow: 'shadow-[0_0_25px_rgba(168,85,247,0.9)] ring-1 ring-purple-300',
        };
      } else {
        bannerConfig = {
          comboCount: combo,
          ...
          glowShadow: 'shadow-[0_0_20px_rgba(249,115,22,0.8)] ring-1 ring-amber-300',
        };
      }
      ...
      bannerConfig = {
        ...
        glowShadow: 'shadow-[0_0_15px_rgba(245,158,11,0.7)]',
      };
    }
  ...
  const getGlowIntensity = (c: number) => {
    if (c >= 10) {
      return {
        containerGlow: 'shadow-[0_0_20px_rgba(245,158,11,0.8)] scale-105 ring-2 ring-amber-300',
        textGlow: 'text-amber-100 font-bold',
        flameClass: 'text-amber-300 fill-amber-400 animate-spin',
      };
    }
    if (c >= 7) {
      return {
        containerGlow: 'shadow-[0_0_16px_rgba(168,85,247,0.8)] scale-105 ring-1 ring-purple-400/80',
        textGlow: 'text-purple-100 font-bold',
        flameClass: 'text-purple-300 fill-purple-400',
      };
    }
    if (c >= 4) {
      return {
        containerGlow: 'shadow-[0_0_12px_rgba(249,115,22,0.7)] scale-105',
        textGlow: 'text-slate-950 font-black',
        flameClass: 'text-orange-400 fill-amber-400',
      };
    }
    if (c >= 2) {
      return {
        containerGlow: 'shadow-[0_0_8px_rgba(245,158,11,0.5)]',
        textGlow: 'text-slate-950 font-bold',
  ...
  if (compact) {
    ...
    return (
      <div className="fixed top-12 left-1/2 -translate-x-1/2 z-50 pointer-events-none w-11/12 max-w-sm gpu-accelerated">
        <div
          className={`bg-gradient-to-r ${activeBanner.bgGradient} border-2 ${activeBanner.borderColor} ${activeBanner.glowShadow} rounded-2xl px-3 py-1.5 flex items-center justify-between gap-2 shadow-lg transition-all duration-200 scale-100`}
        >
  ...
  return (
    <div className="relative w-full max-w-2xl mx-auto gpu-accelerated">
      {/* Prominent Floating Milestone Overlay Banner */}
      {activeBanner && (
        <div className="absolute -top-12 left-1/2 -translate-x-1/2 z-50 pointer-events-none w-11/12 max-w-md gpu-accelerated">
          <div
            className={`bg-gradient-to-r ${activeBanner.bgGradient} border-2 ${activeBanner.borderColor} ${activeBanner.glowShadow} rounded-2xl px-4 py-2 flex items-center justify-between gap-3 shadow-lg transition-all duration-200 scale-100`}
          >
      ...
      {/* Main HUD Bar */}
      <div className="w-full flex items-center justify-between bg-slate-900/95 border border-slate-800 rounded-xl px-3 py-1.5 shadow-md transition-all duration-200">
        ...
        <div className="flex items-center gap-2">
          <div className={`p-1.5 rounded-lg flex items-center justify-center transition-colors duration-200 ${
            combo > 0 ? 'bg-amber-500/20 text-amber-400' : 'bg-slate-800 text-slate-600'
          }`}>
          ...
          <div>
            ...
            <div className="flex items-baseline gap-1">
              <span className={`font-arcade text-lg font-black tracking-wider ${
                combo >= 10
                  ? 'text-yellow-300'
                  : combo >= 7
                  ? 'text-purple-300'
            ...
            return (
              <div
                ...
                className={`h-2 rounded-full transition-colors duration-200 ${
                  ...
                  isReached
                    ? step >= 15
                      ? 'bg-amber-300 ring-1 ring-amber-300'
                      : step >= 10
                      ? 'bg-yellow-300'
                      : step >= 7
                      ? 'bg-purple-400'
        ...
        <div className="flex items-center gap-2">
          {lastBonusPoints && lastBonusPoints > 0 ? (
            <div className="bg-emerald-500 text-slate-950 font-arcade font-bold text-xs px-2 py-0.5 rounded-lg shadow-sm">
              +{lastBonusPoints} BONUS!
            </div>
          ) : null}

          <div className={`px-2.5 py-1 rounded-xl border text-xs font-arcade font-black tracking-wider bg-gradient-to-r ${colorClass} ${borderClass} ${containerGlow} flex items-center gap-1`}>
