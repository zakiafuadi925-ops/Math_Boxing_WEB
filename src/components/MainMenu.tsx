...
import { loadDailyChallengeState, DailyChallengeState } from '../utils/dailyChallenges';
import { LoginModal } from './LoginModal';
import { PlayerProfile } from '../lib/firebase';
import { User, LogIn, ShieldCheck } from 'lucide-react';

interface MainMenuProps {
  onStartGame: (mode: GameMode, category: QuestionCategory, aiDiff?: 'easy' | 'normal' | 'hard', roomCode?: string) => void;
...
const DEFAULT_MOCK_LEADERBOARD: LeaderboardEntry[] = [
  { id: 'lb-1', name: 'Budi Math-Champ', avatar: '🥇', score: 480, winRate: 96, badge: 'Grandmaster', status: 'online', categoryLabel: 'Aritmatika' },
  { id: 'lb-2', name: 'Siti Speed-Math', avatar: '🥈', score: 410, winRate: 92, badge: 'Master', status: 'in_match', categoryLabel: 'Aljabar' },
  { id: 'lb-3', name: 'Rizky KO-Striker', avatar: '🥉', score: 360, winRate: 88, badge: 'Diamond', status: 'online', categoryLabel: 'Akar Pangkat' },
  { id: 'lb-4', name: 'Ahmad Speed-Calc', avatar: '🥊', score: 310, winRate: 85, badge: 'Platinum', status: 'offline', categoryLabel: 'Fisika Dasar' },
  { id: 'lb-5', name: 'Dewi Formula-Pro', avatar: '⚡', score: 275, winRate: 81, badge: 'Gold', status: 'online', categoryLabel: 'Geometri' },
  { id: 'lb-6', name: 'Fajar Smart-Kid', avatar: '🧠', score: 230, winRate: 77, badge: 'Silver', status: 'in_match', categoryLabel: 'Counting' },
  { id: 'lb-7', name: 'Nadia Math-Ninja', avatar: '🥷', score: 195, winRate: 74, badge: 'Silver', status: 'offline', categoryLabel: 'Semua Materi' },
];
...
export const MainMenu: React.FC<MainMenuProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'arena' | 'stats' | 'leaderboard' | 'skins'>('arena');
  const [showHowToPlay, setShowHowToPlay] = useState(false);
  const [showPrivateModal, setShowPrivateModal] = useState(false);
  const [showDailyModal, setShowDailyModal] = useState(false);
  const [showLoginModal, setShowLoginModal] = useState(false);
  const [dailyState, setDailyState] = useState<DailyChallengeState>(() => loadDailyChallengeState());

  useEffect(() => {
    setDailyState(loadDailyChallengeState());
  }, [activeTab, showDailyModal]);

  const dailyCompletedCount = dailyState.challenges.filter((c) => c.isCompleted).length;
  const dailyTotalCount = dailyState.challenges.length;
  const hasUnclaimedDaily = dailyState.challenges.some((c) => c.isCompleted && !c.isClaimed);
    ...
    const currentUserEntry: LeaderboardEntry = {
      ...
      winRate: Math.round(
        fullMatchHistory.length > 0
          ? (fullMatchHistory.filter((m) => m.result === 'win').length / fullMatchHistory.length) * 100
          : 0
      ),
      badge: userRankBadge,
      isCurrentUser: true,
      status: 'online',
      categoryLabel: selectedCategory === 'all' ? 'Semua Materi' : selectedCategory.toUpperCase(),
    };
  ...
  const currentUserRank = computedLeaderboard.find((e) => e.isCurrentUser)?.rank || '-';
  ...
  const statsSummary = useMemo(() => {
    if (!fullMatchHistory || fullMatchHistory.length === 0) {
      return {
        ...
        trendData: [],
        categoryData: [],
        improvementMessage: 'Belum ada data pertandingan. Selesaikan match pertamamu di Arena untuk mencatat tren statistik!',
      };
    }
    ...
    const trendData = chronological.map((item, index) => {
      ...
      return {
        ...
        result: item.result === 'win' ? 'Menang' : item.result === 'draw' ? 'Seri' : 'Kalah',
    ...
    const totalScore = fullMatchHistory.reduce((sum, m) => sum + (m.p1Score || 0), 0);

    // Category accuracy breakdown
    const categoryStats: Record<string, { totalAcc: number; count: number; label: string }> = {
    ...
    const categoryData = Object.keys(categoryStats)
      ...
      .map((key) => ({
        ...
        accuracy: Math.round(categoryStats[key].totalAcc / categoryStats[key].count),
    ...
    if (trendData.length >= 3) {
      ...
      const recentAvg = Math.round(recent3.reduce((s, i) => s + i.accuracy, 0) / recent3.length);
      const earlierAvg = Math.round(earlier.reduce((s, i) => s + i.accuracy, 0) / earlier.length);
  ...
  return (
    <div className="w-full max-w-xl mx-auto flex flex-col items-center justify-center p-4">
      ...
      <div className="w-full flex items-center justify-between mb-3 bg-slate-900/80 border border-slate-800 rounded-2xl px-4 py-2.5 backdrop-blur">
        ...
        >
          {currentUser?.photoURL ? (
            ...
            <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-blue-500 via-indigo-500 to-amber-400 flex items-center justify-center font-arcade font-bold text-white text-xs shadow border border-blue-400/50">
              {currentUser ? currentUser.displayName.charAt(0).toUpperCase() : 'G'}
            </div>
          )}
          <div>
            <div className="flex items-center gap-1">
              <span className="font-bold text-xs text-slate-100 truncate max-w-[110px]">
                {currentUser ? currentUser.displayName : playerName || 'Guest Player'}
              </span>
              {currentUser ? (
            ...
            <span className="block text-[10px] text-emerald-400 font-medium">
              {currentUser ? '● Terverifikasi Google' : '● Masuk Google (Simpan Score)'}
            </span>
          </div>
        </button>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            ...
            <span className="hidden sm:inline text-xs font-bold font-arcade">DAILY</span>
            {hasUnclaimedDaily ? (
      ...
      <div className="flex items-center justify-center gap-1.5 my-3 p-1 bg-slate-900 border border-slate-800 rounded-xl w-full">
        ...
        >
          <Trophy className="w-4 h-4 text-amber-950 fill-amber-950/20" /> LEADERBOARD
        </button>
        <button
          onClick={() => {
            audio.playClick();
        ...
        <div className="w-full space-y-3">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="text-left">
                ...
                <span className="text-xs text-slate-300 font-medium">
                  {dailyCompletedCount} / {dailyTotalCount} Selesai • Klaim +100 - +200 PTS!
                </span>
              </div>
            </div>
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div className="text-left">
                <span className="font-arcade text-xl font-black block">QUICK MATCH ONLINE</span>
                <span className="text-xs font-bold text-slate-800">Cari lawan cepat & adu speed math 1v1!</span>
              </div>
            </div>
            <span className="font-arcade text-sm px-3 py-1 bg-slate-950 text-amber-400 rounded-lg">PLAY</span>
          </button>
          ...
          <div className="w-full p-4 bg-slate-900 border-2 border-slate-800 rounded-2xl shadow-xl flex flex-col gap-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div className="text-left">
                  <span className="font-arcade text-lg text-slate-100 block">LATIHAN VS AI BOT</span>
                  <span className="text-xs text-slate-400">Asah refleks hitung tanpa koneksi lawan</span>
                </div>
              </div>
            ...
            <div className="flex items-center gap-2 pt-2 border-t border-slate-800">
              <span className="text-xs font-semibold text-slate-400">Kesulitan Bot:</span>
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div className="text-left">
                <span className="font-arcade text-lg text-slate-100 block">PRIVATE ROOM</span>
                <span className="text-xs text-slate-400">Main bareng teman via Kode Kamar</span>
              </div>
            </div>
          ...
          <div className="w-full bg-slate-900/90 border-2 border-slate-800 rounded-2xl p-4 my-2">
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-[10px] text-slate-500 font-medium">5 Match Terakhir</span>
            </div>

            {matchHistory.length === 0 ? (
              <div className="bg-slate-950 border border-slate-800 rounded-xl p-4 text-center">
                <p className="text-xs text-slate-400">
                  Belum ada riwayat pertandingan. Mulai bertanding untuk mencatat skor!
                </p>
              </div>
            ) : (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="flex items-center gap-2">
                            ...
                            >
                              {isWin ? 'MENANG (KO)' : isDraw ? 'SERI' : 'KALAH'}
                            </span>
                          ...
                          <span className="text-[10px] text-slate-500 block capitalize">
                            Mode: {item.mode === 'practice' ? 'Latihan AI' : item.mode === 'quick_match' ? 'Quick Online' : 'Private Room'}
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="text-right">
                        <span className="font-arcade text-base text-slate-100 font-bold block">
                          {item.p1Score} <span className="text-slate-500 text-xs">-</span> {item.p2Score}
                        </span>
        ...
        <div className="w-full space-y-3">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-3 text-center">
              ...
              <span className="text-[10px] text-slate-500 block mt-0.5">Rata-Rata Total</span>
            </div>
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-3 text-center">
              ...
              <span className="text-[10px] text-slate-500 block mt-0.5">Selesai Bertanding</span>
            </div>
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-3 text-center">
              ...
              <span className="text-[10px] text-slate-500 block mt-0.5">Akumulasi Poin</span>
            </div>
          </div>
          ...
          <div className="bg-slate-900/90 border border-amber-500/30 rounded-2xl p-3.5 flex items-center gap-3 shadow-lg">
            ...
            <div>
              <span className="text-xs font-bold text-amber-300 block">ANALISIS PERFORMA KAMU:</span>
              <p className="text-xs text-slate-300 mt-0.5">{statsSummary.improvementMessage}</p>
            </div>
          </div>
          ...
          <div className="bg-slate-900/90 border-2 border-slate-800 rounded-2xl p-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-[10px] text-slate-500 font-medium flex items-center gap-1">
                <Activity className="w-3 h-3 text-emerald-400" /> Recharts Chronological
              </span>
            </div>
            ...
            {statsSummary.trendData.length === 0 ? (
              <div className="bg-slate-950 border border-slate-800 rounded-xl p-8 text-center my-2">
                <p className="text-xs text-slate-400">
                  Belum ada grafik statistik. Mainkan beberapa pertandingan di Arena untuk melihat grafik tren perkembanganmu!
                </p>
              </div>
            ) : (
              <div className="w-full h-52 mt-2">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={statsSummary.trendData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                    <defs>
                      <linearGradient id="lifetimeGrad" x1="0" y1="0" x2="0" y2="1">
                    ...
                    <XAxis dataKey="matchName" stroke="#64748b" fontSize={10} tickLine={false} />
                    ...
                    <Tooltip
                      ...
                      formatter={(value: any, name: any) => {
                        if (name === 'accuracy') return [`${value}%`, 'Akurasi Match'];
                        if (name === 'runningAvg') return [`${value}%`, 'Rata-Rata Kumulatif'];
                        return [value, name];
                      }}
          ...
          {statsSummary.categoryData.length > 0 && (
            <div className="bg-slate-900/90 border-2 border-slate-800 rounded-2xl p-4">
              ...
              <div className="w-full h-40">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={statsSummary.categoryData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#1e293b" />
                    ...
                    <Tooltip
                      ...
                      formatter={(value: any) => [`${value}%`, 'Akurasi Rata-Rata']}
                    />
        ...
        <div className="w-full space-y-3">
          ...
          <div className="bg-slate-900/90 border-2 border-slate-800 rounded-2xl p-4 shadow-xl">
            <div className="flex items-center justify-between mb-2">
              ...
              >
                <RefreshCw className={`w-3.5 h-3.5 ${isRefreshingLeaderboard ? 'animate-spin text-amber-300' : ''}`} />
            ...
            <div className="flex items-center justify-between text-[11px] text-slate-400 border-t border-slate-800/80 pt-2.5">
              ...
              <span className="text-slate-500 font-arcade text-[10px]">60S SUDDEN DEATH ARENA</span>
            </div>
          </div>
          ...
          <div className="bg-gradient-to-r from-amber-950/70 via-slate-900 to-slate-900 border-2 border-amber-500/60 rounded-2xl p-3.5 flex items-center justify-between shadow-lg">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="flex items-center gap-1.5">
                  <span className="font-bold text-sm text-slate-100">{playerName || 'Pemain Kamu'}</span>
                ...
                <div className="flex items-center gap-2 mt-0.5 text-xs text-slate-300">
                  <span className="text-amber-400 font-bold">{lifetimeScore} PTS</span>
                  <span className="text-slate-500">•</span>
                  <span className="text-slate-400 text-[11px]">Kategori: {selectedCategory.toUpperCase()}</span>
                </div>
              </div>
            </div>
            ...
            <div className="text-right">
              <span className="px-2.5 py-1 rounded-lg bg-amber-500/20 border border-amber-500/40 text-amber-300 font-arcade text-[10px] font-bold block">
                {lifetimeScore >= 350 ? 'MASTER' : lifetimeScore >= 150 ? 'DIAMOND' : 'PETINJU KELAS'}
              </span>
          ...
          <div className="grid grid-cols-3 gap-2 py-1">
            ...
            {computedLeaderboard[1] && (
              <div className="bg-slate-900/80 border border-slate-700/80 rounded-xl p-2.5 text-center flex flex-col items-center justify-between relative shadow">
                ...
                <div className="mt-2 text-2xl">{computedLeaderboard[1].avatar}</div>
                <div className="mt-1 w-full truncate">
                  ...
                  <span className="text-[9px] text-slate-500 block">{computedLeaderboard[1].badge}</span>
                </div>
              </div>
            )}
            ...
            {computedLeaderboard[0] && (
              <div className="bg-gradient-to-b from-amber-950/90 to-slate-900 border-2 border-amber-400 rounded-xl p-2.5 text-center flex flex-col items-center justify-between relative shadow-xl scale-105 z-10">
                ...
                <div className="mt-2 text-3xl">{computedLeaderboard[0].avatar}</div>
                <div className="mt-1 w-full truncate">
                  ...
                  <span className="text-[9px] text-amber-200/80 font-semibold block">{computedLeaderboard[0].badge}</span>
                </div>
              </div>
            )}
            ...
            {computedLeaderboard[2] && (
              <div className="bg-slate-900/80 border border-amber-900/60 rounded-xl p-2.5 text-center flex flex-col items-center justify-between relative shadow">
                ...
                <div className="mt-2 text-2xl">{computedLeaderboard[2].avatar}</div>
                <div className="mt-1 w-full truncate">
                  <span className="font-bold text-xs text-slate-200 block truncate">
                  ...
                  <span className="text-[9px] text-slate-500 block">{computedLeaderboard[2].badge}</span>
                </div>
              </div>
            )}
          </div>
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-1.5 truncate">
                        <span
                          className={`font-bold text-xs truncate ${
                            entry.isCurrentUser ? 'text-amber-300 font-black' : 'text-slate-200'
                          }`}
                        >
                          {entry.name}
                        </span>
                      ...
                      <div className="flex items-center gap-1.5 text-[10px] text-slate-400 mt-0.5">
                        <span className="flex items-center gap-1">
                          ...
                          />
                          {entry.status === 'online' ? 'Online' : entry.status === 'in_match' ? 'Bertarung' : 'Offline'}
                        </span>
                        <span>•</span>
                ...
                >
                  ...
                  <div className="mt-2">
                    ...
                    <span className="text-[10px] text-slate-400 block mt-0.5">
                      {isUnlocked ? (isSelected ? 'Dipakai Saat Ini' : 'Klik Untuk Pakai') : skin.description}
                    </span>
                  </div>
                </button>
              );
      ...
      {showPrivateModal && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-slate-900 border-2 border-slate-800 rounded-2xl p-6 w-full max-w-sm text-center shadow-2xl">
            <h3 className="font-arcade text-xl text-amber-400 mb-2">MASUKKAN KODE KAMAR</h3>
            <p className="text-xs text-slate-400 mb-4">Buat kode baru atau masukkan kode temanmu</p>
              ...
              <button
                onClick={() => {
                  if (roomInput.trim()) {
                    ...
                    onStartGame('private_room', selectedCategory, undefined, roomInput.trim());
                  }
                }}
          ...
          <div className="bg-slate-900 border-2 border-slate-800 rounded-2xl p-6 w-full max-w-md shadow-2xl space-y-4">
            ...
            <ul className="text-xs text-slate-300 space-y-2.5 leading-relaxed">
              <li className="flex items-start gap-2">
                ...
                <span><b>Durasi 60 Detik:</b> Pertarungan berjalan cepat selama 1 menit (Sudden Death).</span>
              </li>
              <li className="flex items-start gap-2">
                <span className="font-arcade text-amber-400">2.</span>
                <span><b>Jawab Cepat:</b> Ketik angka jawaban pada numpad statis lalu tekan <b>ENTER</b>.</span>
              </li>
              <li className="flex items-start gap-2">
                <span className="font-arcade text-amber-400">3.</span>
                <span><b>Pukul Opponent:</b> Setiap jawaban benar membuat petinjumu memukul lawan (+2 s/d +10 poin)!</span>
              </li>
              <li className="flex items-start gap-2">
                <span className="font-arcade text-amber-400">4.</span>
                <span><b>Penalti Salah:</b> Jika jawaban salah, Numpad akan terkunci selama 1 detik.</span>
              </li>
              <li className="flex items-start gap-2">
                <span className="font-arcade text-amber-400">5.</span>
                <span><b>Pemenang:</b> Pemain dengan akumulasi skor tertinggi saat timer habis memenangkan pertandingan!</span>
              </li>
            </ul>
